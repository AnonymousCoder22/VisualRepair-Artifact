...
def filter_small_compos_by_area(compos, img_w, img_h, min_compo_ratio=0.001):
    """
    Filter out components whose area ratio is too small relative to the source image.
    """
    img_area = img_w * img_h if img_w > 0 and img_h > 0 else 1
    filtered = []

    for compo in compos:
        w = compo.col_max - compo.col_min
        h = compo.row_max - compo.row_min
        area_ratio = (w * h) / img_area
        if area_ratio >= min_compo_ratio:
            filtered.append(compo)
...
# Added in version 3.1
    img_shape = compo_json['img_shape']
    img_h, img_w = img_shape[0], img_shape[1]
    compos = filter_small_compos_by_area(
        compos,
        img_w=img_w,
        img_h=img_h,
        min_compo_ratio=0.001  # Keep this threshold aligned with load_boxes.
    )
