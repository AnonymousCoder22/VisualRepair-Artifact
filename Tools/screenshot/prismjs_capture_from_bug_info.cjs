...
function tryChromiumScreenshot(browserExe, pageUrl, outPath, viewport) {
  ...
  const userDataDir = fs.mkdtempSync(path.join(tmpBase, 'guirepair-prismjs-profile-'));
...
function resolveHtmlEntry(repoDir, htmlArg) {
  ...
  return {
    htmlPath: path.resolve(__dirname, 'code_template', 'PrismJS', 'template_prismjs.html'),
    pagePath: '/code.html'
  };
}
...
async function main() {
  ...
  try {
    ...
    console.log(`[prismjs-screenshot] wrote ${outPath}`);
  } finally {
