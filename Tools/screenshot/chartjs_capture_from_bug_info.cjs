...
function tryChromiumScreenshot(browserExe, pageUrl, outPath, viewport) {
  ...
  const userDataDir = fs.mkdtempSync(path.join(tmpBase, 'guirepair-chartjs-profile-'));
...
function resolveHtmlEntry(repoDir, htmlArg) {
  ...
  return {
    htmlPath: path.resolve(__dirname, 'code_template', 'chartjs', 'template_chartjs.html'),
    pagePath: '/code.html'
  };
}
...
async function main() {
  ...
  try {
    ...
    console.log(`[chartjs-screenshot] wrote ${outPath}`);
  } finally {
